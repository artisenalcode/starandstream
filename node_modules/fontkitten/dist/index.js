...
var CFFDict = class {
	...
	decodeOperands(type, stream, ret, operands) {
		...
		else if (type.decode != null) return type.decode(stream, ret, operands);switch (type) {
			case "number":
			case "offset":
	...
	*/
	getVariation(settings) {
		...
		const coords = this.fvar.axis.map((axis) => {
			...
			if (axisTag in settings) return Math.max(axis.minValue, Math.min(axis.maxValue, settings[axisTag]));return axis.defaultValue;
		});
...
var WOFF2Font = class extends TTFFont {
	...
	_getBaseGlyph(glyph, characters = []) {
		...
		} else return super._getBaseGlyph(glyph, characters);return this._glyphs[glyph];
	}
	_transformGlyfTable() {
		this.#decompress();
