...
var Diff = class {
	...
	equals(left, right, options8) {
		if (options8.comparator) return options8.comparator(left, right);return left === right || !!options8.ignoreCase && left.toLowerCase() === right.toLowerCase();
	}
	removeEmpty(array2) {
		const ret = [];
...
${frame}`;
	if (shouldHighlight) return defs.reset(frame);return frame;
}
