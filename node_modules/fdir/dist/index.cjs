...
function joinPathWithRelativePath(root, options) {
	return function(filename, directoryPath) {
		...
		if (sameRoot) return directoryPath.slice(root.length) + filename;return convertSlashes((0, path.relative)(root, directoryPath), options.pathSeparator) + options.pathSeparator + filename;
	};
}
...
function build$5(options) {
	...
	else if (onlyCounts) return pushFileCount;return pushFile;
}
...
function build$1(options, isSynchronous) {
	...
	else if (maxFiles) return isSynchronous ? limitFilesSync : limitFilesAsync;return isSynchronous ? defaultSync : defaultAsync;
}
