...
module.exports = core => {
  ...
  Stream.prototype = {
    ...
    trypopnumber: function() {
      var tok = this.peek();
      if (this.isNumberRe.test(tok)) return parseFloat(this.pop()) ?? null;return null;
    },
    trypopvarref: function() {
      var tok = this.peek();
      if (null == tok) return null;
      if ('$' === tok.charAt(0)) return this.pop().substr(1) ?? null;return null;
    },
    position: function() {
  ...
  function relativeLocationPath(lhs, stream, a, isOnlyRootOk) {
    ...
    while (op = stream.trypop(['/', '//'])) {
      ...
      if (null == rhs && '/' === op && isOnlyRootOk) return lhs;isOnlyRootOk = false;
      if (null == rhs)
        throw new XPathException(XPathException.INVALID_EXPRESSION_ERR,
                                 'Position ' + stream.position() +
  ...
  function pathExpr(stream, a) {
    ...
    if (filter === rel) return rel;return a.node('PathExpr', rel);
  }
  ...
  var fn = {
    ...
    'substring': function substring(string, start, optEnd) {
      ...
      if (iEnd == null)
        return sString.substr(iStart - 1);return sString.substr(iStart - 1, iEnd);
    },
