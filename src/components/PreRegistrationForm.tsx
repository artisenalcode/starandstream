...
export default function PreRegistrationForm({ courseName, showFeeNotice = false }: PreRegistrationFormProps) {
  ...
  return (
    <div className="bg-boho-wheat/5 p-8 border border-boho-wheat/40 max-w-2xl mx-auto">
      <h3 className="text-3xl font-bold text-boho-navy mb-4 uppercase tracking-wide">Pre-register for {courseName}</h3>
      <p className="text-boho-navy/80 mb-8 font-sans">
        Join the waitlist for our upcoming cohort in Afton, WY. 
        {showFeeNotice && <span className="block mt-2 font-semibold text-boho-gold">Note: An RSVP fee of $10 will be required to secure your spot once registration officially opens.</span>}
      </p>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-boho-navy mb-2">Full Name</label>
            <input 
              type="text" 
              id="name" 
              required
          ...
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-boho-navy mb-2">Email Address</label>
            <input 
              type="email" 
              id="email" 
              required
        ...
        <div>
          <label htmlFor="role" className="block text-sm font-medium text-boho-navy mb-2">I am a...</label>
          <select 
            id="role" 
            required
        ...
        <div>
          <label htmlFor="interest" className="block text-sm font-medium text-boho-navy mb-2">Why are you interested in this course?</label>
          <textarea 
            id="interest" 
            rows={4}
        ...
        {status === 'success' && (
          <p className="text-boho-denim text-center mt-4 font-medium">Thank you. We take a disciplined approach to our growth—expect an update from us soon.</p>
        )}
      </form>
    </div>
  );
